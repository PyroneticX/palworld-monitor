...
class BanlistManager:
    ...
    def _detect_banlist_path(self):
        """Detect the banlist.txt file path based on server executable path.

        Returns:
            str or None: Path to banlist.txt or None if not found
        """
        try:
            ...
            possible_paths = [
                ...
                os.path.join(server_dir, "Pal", "Saved", "SaveGames", "0", "banlist.txt"),
        ...
        with self._lock:
            try:
                if not os.path.exists(self.banlist_path):
                    logging.debug(f"Banlist file does not exist at {self.banlist_path}, returning empty list")
                    return []
                banned_steam_ids = []
                ...
                logging.debug(f"Loaded {len(banned_steam_ids)} banned players from {self.banlist_path}")
