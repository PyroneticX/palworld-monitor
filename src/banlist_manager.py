    ...
    def get_banned_players(self) -> List[str]:
        ...
        try:
            if not os.path.exists(self.banlist_path):
                logging.info(f"Banlist file does not exist at {self.banlist_path}, returning empty list")
                return []
            ...
            logging.info(f"Loaded {len(banned_steam_ids)} banned players from {self.banlist_path}")
